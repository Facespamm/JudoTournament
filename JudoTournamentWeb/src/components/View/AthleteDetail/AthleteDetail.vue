<template>
    <div class="athlete-detail-container">
        ...
        <div class="athlete-detail-header">
            ...
            <h1>{{ t("athleteDetail.title") }}</h1>
        </div>

        <!-- Лоадер -->
        <div v-if="isLoading" class="loading">
            <div class="spinner"></div>
            <p>{{ t("athleteDetail.loading") }}</p>
        </div>

        <!-- Ошибка -->
        <div v-else-if="error" class="error">
            <p>{{ error }}</p>
            <button class="retry-button" @click="loadAthleteDetail">
                {{ t("athleteDetail.retry") }}
            </button>
        </div>
        ...
        <div v-else-if="athlete" class="athlete-info">
            <div class="athlete-main-info">
                ...
                <div v-if="athlete.rank" class="athlete-badge">
                    {{ athlete.rank }}
                </div>
            </div>
            ...
            <div class="athlete-details-grid">
                <div class="detail-section">
                    <h3>{{ t("athleteDetail.mainInfo") }}</h3>
                    <div class="detail-item">
                        <span class="detail-label">{{
                            t("athleteDetail.lastName")
                        }}</span>
                    ...
                    <div class="detail-item">
                        ...
                        <span class="detail-value">{{
                            t("athleteDetail.years", { count: athlete.age })
                        }}</span>
                    </div>
                </div>

                <div class="detail-section">
                    <h3>{{ t("athleteDetail.sportInfo") }}</h3>
                    <div class="detail-item">
                        <span class="detail-label">{{
                            t("athleteDetail.rank")
                        }}</span>
                        <span class="detail-value">{{ athlete.rank }}</span>
                    </div>
                    <div class="detail-item">
                        <span class="detail-label">{{
                            t("athleteDetail.club")
                        }}</span>
                        <span class="detail-value">{{
                            athlete.club_name
                        }}</span>
                    ...
                    <div class="detail-item">
                        <span class="detail-label">{{
                            t("athleteDetail.medical")
                        }}</span>
                        <span class="detail-value">{{
                            athlete.medical_check
                                ? t("athleteDetail.passed")
                                : t("athleteDetail.notPassed")
                        }}</span>
                    </div>
                </div>

                <div class="detail-section">
                    <h3>{{ t("athleteDetail.contactInfo") }}</h3>
                    <div v-if="athlete.phone" class="detail-item">
                        <span class="detail-label">{{
                            t("athleteDetail.phone")
                        }}</span>
...
const getGenderText = (gender) => {
    const genders = {
        M: t("athleteDetail.male"),
        М: t("athleteDetail.male"),
        F: t("athleteDetail.female"),
        Ж: t("athleteDetail.female"),
        MALE: t("athleteDetail.male"),
        FEMALE: t("athleteDetail.female"),
    };
...
const formatDate = (dateString) => {
    if (!dateString) return t("athleteDetail.notSpecified");
...
const loadAthleteDetail = async () => {
    ...
    try {
        ...
        if (isNaN(athleteId)) {
            throw new Error(t("athleteDetail.invalidId"));
        }
        const response = await fetchAthleteDetail(athleteId);
        if (response.success) {
            athlete.value = response.athlete;
        } else {
            throw new Error(t("athleteDetail.loadFailed"));
        }
    } catch (err) {
        error.value =
            err instanceof Error ? err.message : t("athleteDetail.loadError");
    } finally {
