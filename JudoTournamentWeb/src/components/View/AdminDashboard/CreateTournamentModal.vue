...
import { createTournament } from '@/components/View/AdminDashboard/fetchAdmin.js' // ← твой сервис, как в клубах
...
const submit = async () => {
  ...
  try {
    ...
    emit('submit', { success: false, error: error.message || 'Произошла неизвестная ошибка' })
  } finally {
    isLoading.value = false
  }
}
...
.modal-button.cancel:hover:not(:disabled) { background: #e8e8e8; }
.modal-button.submit {
...
.modal-button.submit:hover:not(:disabled) {
  ...
  box-shadow: 0 4px 12px rgba(200,155,60,.3);
}
