            ...
            <form @submit.prevent="weighAthlete">
                <div class="form-fields">
                    ...
                    <div class="form-group">
                        ...
                        <span v-if="errors.tournament_id" class="error">{{
                    ...
                    <div class="form-group athlete-selection-group">
                        ...
                        <div class="athletes-table-wrapper">
                            ...
                            <table v-else class="weighings-athletes-table">
                                ...
                                <tbody>
                                    <tr
                                        ...
                                        :class="{
                                            'selected-row':
                                                weighingForm.athlete_id ===
                                                (ath.athlete_id || ath.id),
                                        }"
                                    >
                                        <td>
                                            <input
                                        ...
                                        <td>
                                            {{
                                                formatBirthDate(ath.birth_date)
                                            }}
                                        </td>
                                        <td>{{ ath.age ?? "—" }}</td>
                                        <td>{{ ath.club_name || "—" }}</td>
                            ...
                            <div
                                v-if="
                                    !isLoadingAthletes &&
                                    filteredCategoryAthletes.length === 0 &&
                                    searchAthleteQuery
                                "
                                class="no-athletes-message"
                            >
                        ...
                        <span v-if="errors.athlete_id" class="error">{{
                            errors.athlete_id
                        }}</span>
                    </div>
                    ...
                    <div class="form-grid-double">
                        <div class="form-group">
                            ...
                            <input
                                ...
                                :disabled="isLoading"
                            />
                            <span v-if="errors.weight" class="error">{{
                                errors.weight
                            }}</span>
                        </div>
...
const filteredCategoryAthletes = computed(() => {
    ...
    if (searchAthleteQuery.value.trim()) {
        ...
        list = list.filter((ath) => {
            ...
            return (
                name.includes(q) ||
                String(ath.athlete_id || ath.id || "").includes(q)
            );
        });
    }
    return list;
...
const fetchTournaments = async () => {
    try {
        ...
        return {
            success: true,
            data: Array.isArray(data) ? data : data.tournaments || [],
        };
    } catch (e) {
        console.error(e);
...
const fetchCategoryAthletes = async (tournamentId, categoryId) => {
    try {
        ...
        const res = await fetch(
            `/api/tournaments/${tournamentId}/categories/${categoryId}/registrations`,
            { headers: API_HEADERS },
        );
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const data = await res.json();
